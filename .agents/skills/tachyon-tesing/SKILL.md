...
- Use `net.javacrumbs.jsonunit.assertj.JsonAssertions.assertThatJson(actual).isEqualTo(expected)` for JSON payload assertions. Assert full JSON payload when possible rather than individual paths. JsonUnit integrates with AssertJ's fluent DSL. For scenarios with dynamic or timestamped fields, use `.whenIgnoringPaths("path.to.field")` or other options like `IGNORING_ARRAY_ORDER`, `IGNORING_EXTRA_FIELDS`, `TREATING_NULL_AS_ABSENT`.
