 ...
 */
public final class SseSocketSubscriber extends QueueSubscriber<SseFrame> implements AutoCloseable {
    ...
    public SseSocketSubscriber(int port, String sessionId, @Nullable String lastEventId, String protocolVersion) {
        this(port, sessionId, lastEventId, protocolVersion, new MessageAggregator<>());
    }

    public SseSocketSubscriber(
            int port,
            String sessionId,
    ...
    /** Awaits the first frame carrying an event id — the priming event a fresh GET stream sends. */
            ...
            try {
                ...
                while ((newline = lineBuf.indexOf("\n")) >= 0) {
                    ...
                    if (idMatcher.matches()) {
                        ...
                        deliver(new SseFrame(pendingId, dataMatcher.group(1)));
                        pendingId = null;
                    }
                }
